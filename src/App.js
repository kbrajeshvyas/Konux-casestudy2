...
class App extends Component {
  ...
  // componentDidMount () {
  //   // axios.get(`./app.json`)
  //   axios.get(`http://konuxdata.getsandbox.com/data`)
  //     .then(res => {
  ...
  filterWeek=()=>{
    // TODO: Write a generic filter for required days
    // let newArr=JSON.parse(JSON.stringify(this.state.data));
    // var date = new Date();
  ...
  render() {
    return (
      <div className="App">
        ...
        <div>
            <LineChart data={this.state.data} elementWidth={600} elementHeight={250}   />
        </div>
        <div>
            <button onClick={this.filterWeek.bind(this)}>Last One Week Data</button>
            <button onClick={this.filterLastWeek.bind(this)}>Last Two Week Data</button>
        </div>
      </div>
    );
  }
